...
//        fetchData(from: url){ result, error in
//            if let error = error {
//                print("Ошибка при вызове fetch data \(error)")
...
//        }
        ...
        APIServiceWithAlamofire.shared.fetchData(from: url) {result in
            switch result {
    ...
    func updateUI(with hero: SuperHero){
        ...
        //тут кстати все работы с UI должны выполняться с main потока а не с фонового
    ...
    func loadImage(from url: URL) {
        //старый метод загрузки
//        URLSession.shared.dataTask(with: url) { data, _, error in
//            guard let data = data, error == nil else {
//                print("Ошибка изображения: \(String(describing: error?.localizedDescription))")
//                return
//            }
...
//        }.resume()
        ...
        //ТАК ПРОСТО????
        //просто внутри imageView добавили функцию???
